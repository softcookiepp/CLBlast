...
#if 1
	auto device_a = device->allocateBuffer(host_a);
	auto device_b = device->allocateBuffer(host_b);
	auto device_c = device->allocateBuffer(host_c);
#else
...
#if 1
	auto status =
			clblast::Gemm(clblast::Layout::kRowMajor, clblast::Transpose::kNo, clblast::Transpose::kNo, m, n, k, alpha,
										device_a, 0, a_ld, device_b, 0, b_ld, beta, device_c, 0, c_ld, device);
#else
	auto queue_plain = queue();
	auto status =
			clblast::Gemm(clblast::Layout::kRowMajor, clblast::Transpose::kNo, clblast::Transpose::kNo, m, n, k, alpha,
										device_a(), 0, a_ld, device_b(), 0, b_ld, beta, device_c(), 0, c_ld, &queue_plain, &event);
#endif
