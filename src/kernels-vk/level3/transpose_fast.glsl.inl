...
void main()
{
	...
	for (int _w_two = 0; _w_two < TRA_WPT; _w_two += 1) {
		...
		#elif TRA_WPT == 8
			Multiply(result[0], alpha, results[_w_two][0]);
			Multiply(result[1], alpha, results[_w_two][1]);
		#elif TRA_WPT == 16
			Multiply(result[0], alpha, results[_w_two][0]);
			Multiply(result[1], alpha, results[_w_two][1]);
			Multiply(result[2], alpha, results[_w_two][2]);
			Multiply(result[3], alpha, results[_w_two][3]);
		#endif
