...
// buffer definitions, used globally throughout the thingy
...
#endif
	// alm is going to be globally declared. there is no other way :c
	//LOCAL_PTR realM* alm,
...
#endif
	// local pointers cannot be passed as args in GLSL
	//LOCAL_PTR realN* blm,
...
#if SA == 0 && GEMMK == 0
INLINE_FUNC realM GlobalToPrivateA(
#if USE_BDA
	const __global realM* restrict agm,
#endif
	const int _mi,
	const int kSizeM, const int idk, const int kwg)
{
	// Computes the indices based on strided/non-strided access
		...
		int mg = _mi + int(gl_LocalInvocationID[0])*(MWI/VWM);
	#elif STRM == 1
	...
	// Computes the indices for the global memory
	int idm = mg + int(GetGroupID0()) * (MWG/VWM);
...
// Same as above, but now for the B input matrix
#if SB == 0 && GEMMK == 0
INLINE_FUNC realN GlobalToPrivateB(
#if USE_BDA
	const __global realN* restrict bgm,
#endif
	const int _ni, const int kSizeN, const int idk)
{
	// Computes the indices based on strided/non-strided access
		...
		uint ng = _ni + gl_LocalInvocationID[1]*(NWI/VWN);
	#elif STRN == 1
		uint ng = gl_LocalInvocationID[1] + _ni*NDIMC;
	#endif

	// Computes the indices for the global memory
	uint idn = ng + GetGroupID1() * (NWG/VWN);
...
INLINE_FUNC realN GlobalToPrivateA2D(
#if USE_BDA
	...
	const int tid_y, const int _ni, const int kSizeK, const int idk, const int _ki)
{
		...
		// oh goodness, what to do about this?
		// I am honestly unsure of how it should be done.
		// Will just temporarily comment this out for now, then fix by trial and error later
		...
		const int a_index = (tid_y * NWI + _ni) * kSizeK + idk + _ki * VWN;
			...
			return vload2_single_alignment(a_index, agm);
		#elif VWN == 4
			return vload4_single_alignmen(a_index, agm);
		#elif VWN == 8
			// these are not implemented in glsl
...
// Same as above, but now for the B input matrix
INLINE_FUNC realM GlobalToPrivateB2D(
#if USE_BDA
	...
	const int tid_x, const int _mi, const int kSizeN, const int idk, const int _ki)
{
		...
		// postponed until that silliness is figured out
		...
		const int b_index = (idk + _ki) * kSizeN + tid_x * MWI + _mi * VWM;
			...
			return vload2_single_alignment(b_index, bgm);
		#elif VWM == 4
			//return vload4(0, b_ptr + b_index);
			return vload4_single_alignment(b_index, bgm);
		#elif VWM == 8
...
#if SA == 1
INLINE_FUNC realM LocalToPrivateA(
	//LOCAL_PTR realM* alm,
...
// Same as above, but now for the B input matrix
#if SB == 1
INLINE_FUNC realN LocalToPrivateB(
	//LOCAL_PTR realN* blm,
...
INLINE_FUNC void StoreResults(
	#if USE_BDA
		__global realM* cgm,
	#endif
	realM c_value, const int _mi, const int _ni, const int kSizeM, const real alpha, const real beta)
{
	#if STRM == 0
		uint mg = _mi + gl_LocalInvocationID[0]*(MWI/VWM);
	#elif STRM == 1
		uint mg = gl_LocalInvocationID[0] + _mi*MDIMC;
	#endif
	#if STRN == 0
		uint ng = _ni + gl_LocalInvocationID[1]*NWI;
	#elif STRN == 1
		uint ng = _ni%VWN + gl_LocalInvocationID[1]*VWN + (_ni/VWN)*VWN*NDIMC;
	#endif
	uint idm = mg + GetGroupID0() * (MWG/VWM);
	uint idn = ng + GetGroupID1() * NWG;
	uint index = idn*(kSizeM/VWM) + idm;
	...
	)
{
	...
	realM cpm[NWI*(MWI/VWM)]; // NWI * MWI
		...
		#endif
		const int tid_x = gl_LocalInvocationID[0] + MDIMC * GetGroupID0();
		const int tid_y = gl_LocalInvocationID[1] + NDIMC * GetGroupID1();
	#endif

	// Combined thread identifier (to disable caching)
	#if SA == 1 || SB == 1
		int tid = gl_LocalInvocationID[0] + MDIMC*gl_LocalInvocationID[1];
	#endif
