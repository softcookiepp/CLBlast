...
void Xtrsm<T>::TrsmColMajor(const Side side, const Triangle triangle, const Transpose a_transpose,
	...
	auto diagonal_invert_event = Event(this->device_());
	...
	if (side == Side::kLeft) {
		...
		if (condition) {
			for (auto i = size_t{0}; i < m; i += block_size) {
				...
				auto gemm1_event = Event(this->device_());
				...
				auto gemm2_event = Event(this->device_());
	...
	device_()->enqueueCopyBuffer(b_buffer(), x_buffer(), 0, 0, b_size*sizeof(T));
