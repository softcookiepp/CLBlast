...
void PopulateConfigurations(const std::vector<Parameter>& parameters, const std::vector<size_t>& local_size_base,
                            ...
                            const LocalMemSizeInfo& local_mem_size_info, const std::vector<size_t>& max_work_item_sizes,
  ...
  if (index == parameters.size()) {
    if (ValidConfiguration(config, local_mem_max, constraints, local_mem_size_info, local_size_base, mul_local_config,
                           div_local_config, max_work_item_sizes, max_work_group_size)) {
  ...
  for (auto& value : parameter.second) {
    ...
    PopulateConfigurations(parameters, local_size_base, mul_local_config, div_local_config, index + 1, config_copy,
                           configuration, local_mem_max, constraints, local_mem_size_info, max_work_item_sizes,
                           max_work_group_size);
  }
}
...
bool ValidConfiguration(const Configuration& config, const size_t local_mem_max, const Constraints& constraints,
  ...
  const auto local = SetThreadConfiguration(config, local_size_base, mul_local_config, div_local_config);
