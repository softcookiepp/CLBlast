...
void SetBlockSize(const size_t value, const Device& device) {
	const auto override_status =
			OverrideParameters(device(), "TrsvRoutine", PrecisionValue<T>(), {{"TRSV_BLOCK_SIZE", value}});
...
void RunTrsvRoutine(const size_t block_size, Queue& queue, const std::vector<Buffer<T>>& buffers) {
	...
	auto status = Trsv<T>(Layout::kRowMajor, Triangle::kLower, Transpose::kNo, Diagonal::kNonUnit, size, buffers[0](), 0,
												size,								// A matrix
	...
	try {
		...
		switch (clblast::GetPrecision(command_line_args)) {
			case clblast::Precision::kSingle:
				clblast::TuneXtrsv<float>(argc, argv);
				break;
			case clblast::Precision::kDouble:
				clblast::TuneXtrsv<double>(argc, argv);
				break;
			case clblast::Precision::kComplexSingle:
				clblast::TuneXtrsv<float2>(argc, argv);
				break;
