...
// =================================================================================================
// tart doesn't have any event-related stuff...should it?
// for now, we should just disable this to be safe
#if 0
...
class Program {
	// Tart has an existing handler for OpenCL programs (at least those generated by clspv)
 ...
 public:
	...
	// assign device and source, compilation is done later
	mDevice = device;
	mSource = source;
	}

	// Binary-based constructor with memory management
	// Device and context are one and the same, for Tart
	explicit Program(tart::device_ptr device, std::vector<uint32_t>& binary) {
	// assign device
	mDevice = device;
	// load the shader spv
	tart::shader_module_ptr shaderModule = device->loadShader(binary);
	mClProgram = device->createCLProgram(shaderModule);
	}

	// Clean-up
	~Program() {
		// we shouldn't actually need to do anything, since Vulkan isn't buggy like OpenCL lol
	}
	...
	void Build(std::vector<std::string>& options) {
		// it should already be built...but do we need more options???
		// It isn't well-documented what sort of options are actually passed
		// OK	yeah
		// this is where some compile-time definitions may be used.
		// so we actually need to build them here...
		// this is obviously not possible if the Program was initialized via SPIR-V
		// TODO: figure out how to add the options
	tart::shader_module_ptr shaderModule = device->compileCL(source);
	mClProgram = device->createCLProgram(shaderModule);
#if 0
	// wait, what sort of options are passed? this might complicate things...
	...
	// Tart sadly doesn't have this yet :c
	...
	std::string GetBuildInfo() const {
		...
		return result;
#else
	return "build info not implemented yet lol";
#endif
	}
	...
	std::string GetIR() const {
		...
		CheckError(clGetProgramInfo(program_, CL_PROGRAM_NUM_DEVICES, sizeof(cl_uint), &num_devices, nullptr));

		std::vector<size_t> binSizesInBytes(num_devices, 0);
		CheckError(clGetProgramInfo(program_, CL_PROGRAM_BINARY_SIZES, num_devices * sizeof(size_t), binSizesInBytes.data(),
																nullptr));
		...
		CheckError(clGetProgramInfo(program_, CL_PROGRAM_BINARIES, num_devices * sizeof(char*), out.data(), nullptr));
		return result;
#else
	// TODO: somehow return SPIR-V. Either by stringifying it or otherwise
	return "binary not implemented, teeheee";
#endif
	}
...
#else
	tart::cl_program_ptr operator()() { return mClProgram; }
#endif
};
...
// we won't need this; Vulkan only makes such a distinction within the shader code itself.
...
public:
	...
	void Launch(const Queue& queue, const std::vector<size_t>& global, const std::vector<size_t>& local,
		...
		CheckError(clEnqueueNDRangeKernel(queue(), *null_kernel_, static_cast<cl_uint>(nullRange.size()), nullptr,
																			nullRange.data(), nullptr, 0, nullptr, nullptr));
#endif
	}
