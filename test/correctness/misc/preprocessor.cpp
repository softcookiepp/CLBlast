...
bool TestKernel(const Device& device, const Context& context, const std::string& kernel_name,
								const std::string& kernel_source, const Precision precision) {
	...
	const auto program_ref =
			CompileFromSource(kernel_source, precision, kernel_name, device, context, compiler_options_ref, 2);

	// Compiles the same kernel, but now with the pre-processor enabled
	try {
		auto compiler_options = std::vector<std::string>();
		const auto program = CompileFromSource(kernel_source, precision, kernel_name, device, context, compiler_options, 1);
		return true;
...
size_t RunPreprocessor(int argc, char* argv[], const bool silent, const Precision precision) {
	...
	if (TestArrayToRegisterPromotion()) {
		passed++;
	} else {
		errors++;
	}
...
#include "../src/kernels/level2/xgemv_fast.opencl"
			;
	if (TestKernel(device, context, "XgemvFast", xgemv_sources, precision)) {
		passed++;
	} else {
		errors++;
	}
...
#include "../src/kernels/level3/xgemm_part4.opencl"
			;
	if (TestKernel(device, context, "Xgemm", gemm_sources, precision)) {
		passed++;
	} else {
		errors++;
	}
...
#include "../src/kernels/level3/xgemm_part4.opencl"
			;
	if (TestKernel(device, context, "Xgemm", gemm_sources_gemmk1, precision)) {
		passed++;
	} else {
		errors++;
	}
...
#include "../src/kernels/level3/xgemm_direct_part3.opencl"
			;
	if (TestKernel(device, context, "XgemmDirectTN", gemm_direct_sources, precision)) {
		passed++;
	} else {
		errors++;
	}
