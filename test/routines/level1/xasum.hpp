...
class TestXasum {
 public:
  ...
  static std::vector<std::string> GetOptions() { return {kArgN, kArgXInc, kArgXOffset, kArgAsumOffset}; }
  ...
  static size_t GetSizeAsum(const Arguments<T>& args) { return 1 + args.asum_offset; }
...
// Describes how to run the clBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference1(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    auto queue_plain = queue();
    auto event = cl_event{};
    auto status = clblasXasum<T>(args.n, buffers.scalar, args.asum_offset, buffers.x_vec, args.x_offset, args.x_inc, 1,
...
// Describes how to run the CPU BLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference2(const Arguments<T>& args, BuffersHost<T>& buffers_host, Queue&) {
    cblasXasum(args.n, buffers_host.scalar, args.asum_offset, buffers_host.x_vec, args.x_offset, args.x_inc);
...
// Describes how to run the cuBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference3(const Arguments<T>& args, BuffersCUDA<T>& buffers, Queue&) {
    auto status = cublasXasum(reinterpret_cast<cublasHandle_t>(args.cublas_handle), args.n, buffers.scalar,
                              args.asum_offset, buffers.x_vec, args.x_offset, args.x_inc);
  ...
  static size_t GetResultIndex(const Arguments<T>& args, const size_t, const size_t) { return args.asum_offset; }
