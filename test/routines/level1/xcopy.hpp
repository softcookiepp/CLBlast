...
class TestXcopy {
 public:
  ...
  static std::vector<std::string> GetOptions() { return {kArgN, kArgXInc, kArgYInc, kArgXOffset, kArgYOffset}; }
  ...
  static StatusCode RunRoutine(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto event = cl_event{};
    auto status = Copy<T>(args.n, buffers.x_vec(), args.x_offset, args.x_inc, buffers.y_vec(), args.y_offset,
...
// Describes how to run the clBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference1(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    auto queue_plain = queue();
    auto event = cl_event{};
    auto status = clblasXcopy<T>(args.n, buffers.x_vec, args.x_offset, args.x_inc, buffers.y_vec, args.y_offset,
                                 args.y_inc, 1, &queue_plain, 0, nullptr, &event);
    clWaitForEvents(1, &event);
...
// Describes how to run the CPU BLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference2(const Arguments<T>& args, BuffersHost<T>& buffers_host, Queue&) {
    cblasXcopy(args.n, buffers_host.x_vec, args.x_offset, args.x_inc, buffers_host.y_vec, args.y_offset, args.y_inc);
...
// Describes how to run the cuBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference3(const Arguments<T>& args, BuffersCUDA<T>& buffers, Queue&) {
    auto status = cublasXcopy(reinterpret_cast<cublasHandle_t>(args.cublas_handle), args.n, buffers.x_vec,
                              args.x_offset, args.x_inc, buffers.y_vec, args.y_offset, args.y_inc);
  ...
  static size_t GetFlops(const Arguments<T>& args) { return 1 * args.n; }
  static size_t GetBytes(const Arguments<T>& args) { return (2 * args.n) * sizeof(T); }
};
