...
class TestXdotu {
 public:
  ...
  static std::vector<std::string> GetOptions() {
    return {kArgN, kArgXInc, kArgYInc, kArgXOffset, kArgYOffset, kArgDotOffset};
  }
  ...
  static size_t GetSizeDot(const Arguments<T>& args) { return 1 + args.dot_offset; }
  ...
  static StatusCode RunRoutine(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto event = cl_event{};
    auto status = Dotu<T>(args.n, buffers.scalar(), args.dot_offset, buffers.x_vec(), args.x_offset, args.x_inc,
...
// Describes how to run the clBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference1(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    auto queue_plain = queue();
    auto event = cl_event{};
    auto status = clblasXdotu<T>(args.n, buffers.scalar, args.dot_offset, buffers.x_vec, args.x_offset, args.x_inc,
...
// Describes how to run the CPU BLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference2(const Arguments<T>& args, BuffersHost<T>& buffers_host, Queue&) {
    cblasXdotu(args.n, buffers_host.scalar, args.dot_offset, buffers_host.x_vec, args.x_offset, args.x_inc,
...
// Describes how to run the cuBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference3(const Arguments<T>& args, BuffersCUDA<T>& buffers, Queue&) {
    auto status =
        cublasXdotu(reinterpret_cast<cublasHandle_t>(args.cublas_handle), args.n, buffers.scalar, args.dot_offset,
  ...
  static size_t GetResultIndex(const Arguments<T>& args, const size_t, const size_t) { return args.dot_offset; }

  // Describes how to compute performance metrics
  static size_t GetFlops(const Arguments<T>& args) { return 2 * args.n; }
  static size_t GetBytes(const Arguments<T>& args) { return ((2 * args.n) + 1) * sizeof(T); }
};
