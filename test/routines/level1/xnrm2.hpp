...
class TestXnrm2 {
 public:
  ...
  static std::vector<std::string> GetOptions() { return {kArgN, kArgXInc, kArgXOffset, kArgNrm2Offset}; }
  ...
  static size_t GetSizeNrm2(const Arguments<T>& args) { return 1 + args.nrm2_offset; }
  ...
  static StatusCode RunRoutine(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto event = cl_event{};
    auto status = Nrm2<T>(args.n, buffers.scalar(), args.nrm2_offset, buffers.x_vec(), args.x_offset, args.x_inc,
                          &queue_plain, &event);
...
// Describes how to run the clBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference1(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    auto queue_plain = queue();
    auto event = cl_event{};
    auto status = clblasXnrm2<T>(args.n, buffers.scalar, args.nrm2_offset, buffers.x_vec, args.x_offset, args.x_inc, 1,
...
// Describes how to run the CPU BLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference2(const Arguments<T>& args, BuffersHost<T>& buffers_host, Queue&) {
    cblasXnrm2(args.n, buffers_host.scalar, args.nrm2_offset, buffers_host.x_vec, args.x_offset, args.x_inc);
...
// Describes how to run the cuBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference3(const Arguments<T>& args, BuffersCUDA<T>& buffers, Queue&) {
    auto status = cublasXnrm2(reinterpret_cast<cublasHandle_t>(args.cublas_handle), args.n, buffers.scalar,
                              args.nrm2_offset, buffers.x_vec, args.x_offset, args.x_inc);
  ...
  static size_t GetResultIndex(const Arguments<T>& args, const size_t, const size_t) { return args.nrm2_offset; }
