...
class TestXscal {
 public:
  ...
  static std::vector<std::string> GetOptions() { return {kArgN, kArgXInc, kArgXOffset, kArgAlpha}; }
  ...
  static void SetSizes(Arguments<T>& args, Queue&) { args.x_size = GetSizeX(args); }
  ...
  static StatusCode RunRoutine(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto event = cl_event{};
    auto status = Scal(args.n, args.alpha, buffers.x_vec(), args.x_offset, args.x_inc, &queue_plain, &event);
...
// Describes how to run the clBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference1(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto event = cl_event{};
    auto status =
        clblasXscal(args.n, args.alpha, buffers.x_vec, args.x_offset, args.x_inc, 1, &queue_plain, 0, nullptr, &event);
    clWaitForEvents(1, &event);
...
// Describes how to run the CPU BLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference2(const Arguments<T>& args, BuffersHost<T>& buffers_host, Queue&) {
    cblasXscal(args.n, args.alpha, buffers_host.x_vec, args.x_offset, args.x_inc);
...
// Describes how to run the cuBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference3(const Arguments<T>& args, BuffersCUDA<T>& buffers, Queue&) {
    auto status = cublasXscal(reinterpret_cast<cublasHandle_t>(args.cublas_handle), args.n, args.alpha, buffers.x_vec,
