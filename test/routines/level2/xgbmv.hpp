...
class TestXgbmv {
 public:
  ...
  static std::vector<std::string> GetOptions() {
    return {kArgM,    kArgN,    kArgKL,      kArgKU,      kArgLayout,  kArgATransp, kArgALeadDim,
            kArgXInc, kArgYInc, kArgAOffset, kArgXOffset, kArgYOffset, kArgAlpha,   kArgBeta};
  }
  ...
  static StatusCode RunRoutine(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto status = Gbmv(args.layout, args.a_transpose, args.m, args.n, args.kl, args.ku, args.alpha, buffers.a_mat(),
                       ...
                       args.y_offset, args.y_inc, &queue_plain, &event);
...
// Describes how to run the clBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference1(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto event = cl_event{};
    auto status =
        clblasXgbmv(convertToCLBLAS(args.layout), convertToCLBLAS(args.a_transpose), args.m, args.n, args.kl, args.ku,
                    args.alpha, buffers.a_mat, args.a_offset, args.a_ld, buffers.x_vec, args.x_offset, args.x_inc,
...
// Describes how to run the CPU BLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference2(const Arguments<T>& args, BuffersHost<T>& buffers_host, Queue&) {
    cblasXgbmv(convertToCBLAS(args.layout), convertToCBLAS(args.a_transpose), args.m, args.n, args.kl, args.ku,
               args.alpha, buffers_host.a_mat, args.a_offset, args.a_ld, buffers_host.x_vec, args.x_offset, args.x_inc,
               args.beta, buffers_host.y_vec, args.y_offset, args.y_inc);
...
// Describes how to run the cuBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference3(const Arguments<T>& args, BuffersCUDA<T>& buffers, Queue&) {
    auto status = cublasXgbmv(reinterpret_cast<cublasHandle_t>(args.cublas_handle), args.layout,
                              convertToCUBLAS(args.a_transpose), args.m, args.n, args.kl, args.ku, args.alpha,
