...
class TestXgemv {
 public:
  ...
  static std::vector<std::string> GetOptions() {
    return {kArgM,    kArgN,       kArgLayout,  kArgATransp, kArgALeadDim, kArgXInc,
  ...
  static StatusCode RunRoutine(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto event = cl_event{};
    auto status = Gemv(args.layout, args.a_transpose, args.m, args.n, args.alpha, buffers.a_mat(), args.a_offset,
...
// Describes how to run the clBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference1(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto event = cl_event{};
    auto status =
        clblasXgemv(convertToCLBLAS(args.layout), convertToCLBLAS(args.a_transpose), args.m, args.n, args.alpha,
                    buffers.a_mat, args.a_offset, args.a_ld, buffers.x_vec, args.x_offset, args.x_inc, args.beta,
...
// Describes how to run the CPU BLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference2(const Arguments<T>& args, BuffersHost<T>& buffers_host, Queue&) {
    cblasXgemv(convertToCBLAS(args.layout), convertToCBLAS(args.a_transpose), args.m, args.n, args.alpha,
...
// Describes how to run the cuBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference3(const Arguments<T>& args, BuffersCUDA<T>& buffers, Queue&) {
    auto status = cublasXgemv(reinterpret_cast<cublasHandle_t>(args.cublas_handle), args.layout,
                              convertToCUBLAS(args.a_transpose), args.m, args.n, args.alpha, buffers.a_mat,
                              args.a_offset, args.a_ld, buffers.x_vec, args.x_offset, args.x_inc, args.beta,
  ...
  static size_t GetBytes(const Arguments<T>& args) { return (args.m * args.n + 2 * args.m + args.n) * sizeof(T); }
};
