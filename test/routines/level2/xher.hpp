...
class TestXher {
 public:
  ...
  static std::vector<std::string> GetOptions() {
    return {kArgN, kArgLayout, kArgTriangle, kArgALeadDim, kArgXInc, kArgAOffset, kArgXOffset, kArgAlpha};
  }
  ...
  static size_t GetSizeA(const Arguments<U>& args) { return args.n * args.a_ld + args.a_offset; }
  ...
  static StatusCode RunRoutine(const Arguments<U>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto event = cl_event{};
    auto status = Her(args.layout, args.triangle, args.n, args.alpha, buffers.x_vec(), args.x_offset, args.x_inc,
...
// Describes how to run the clBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference1(const Arguments<U>& args, Buffers<T>& buffers, Queue& queue) {
    auto queue_plain = queue();
    auto event = cl_event{};
    auto status = clblasXher(convertToCLBLAS(args.layout), convertToCLBLAS(args.triangle), args.n, args.alpha,
                             buffers.x_vec, args.x_offset, args.x_inc, buffers.a_mat, args.a_offset, args.a_ld, 1,
...
// Describes how to run the CPU BLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference2(const Arguments<U>& args, BuffersHost<T>& buffers_host, Queue&) {
    cblasXher(convertToCBLAS(args.layout), convertToCBLAS(args.triangle), args.n, args.alpha, buffers_host.x_vec,
              args.x_offset, args.x_inc, buffers_host.a_mat, args.a_offset, args.a_ld);
...
// Describes how to run the cuBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference3(const Arguments<U>& args, BuffersCUDA<T>& buffers, Queue&) {
    auto status = cublasXher(reinterpret_cast<cublasHandle_t>(args.cublas_handle), args.layout,
                             ...
                             args.x_inc, buffers.a_mat, args.a_offset, args.a_ld);
  ...
  static size_t GetFlops(const Arguments<U>& args) { return 3 * args.n * args.n; }
  static size_t GetBytes(const Arguments<U>& args) { return (args.n * args.n + args.n) * sizeof(T); }
};
