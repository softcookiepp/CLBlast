...
class TestXhpr {
 public:
  ...
  static std::vector<std::string> GetOptions() {
    return {kArgN, kArgLayout, kArgTriangle, kArgXInc, kArgAPOffset, kArgXOffset, kArgAlpha};
  }
  ...
  static size_t GetSizeX(const Arguments<U>& args) { return args.n * args.x_inc + args.x_offset; }
  static size_t GetSizeAP(const Arguments<U>& args) { return ((args.n * (args.n + 1)) / 2) + args.ap_offset; }
  ...
  static StatusCode RunRoutine(const Arguments<U>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto status = Hpr(args.layout, args.triangle, args.n, args.alpha, buffers.x_vec(), args.x_offset, args.x_inc,
                      buffers.ap_mat(), args.ap_offset, &queue_plain, &event);
...
// Describes how to run the clBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference1(const Arguments<U>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto event = cl_event{};
    auto status =
        clblasXhpr(convertToCLBLAS(args.layout), convertToCLBLAS(args.triangle), args.n, args.alpha, buffers.x_vec,
                   args.x_offset, args.x_inc, buffers.ap_mat, args.ap_offset, 1, &queue_plain, 0, nullptr, &event);
    clWaitForEvents(1, &event);
...
// Describes how to run the CPU BLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference2(const Arguments<U>& args, BuffersHost<T>& buffers_host, Queue&) {
    cblasXhpr(convertToCBLAS(args.layout), convertToCBLAS(args.triangle), args.n, args.alpha, buffers_host.x_vec,
              args.x_offset, args.x_inc, buffers_host.ap_mat, args.ap_offset);
...
// Describes how to run the cuBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference3(const Arguments<U>& args, BuffersCUDA<T>& buffers, Queue&) {
    auto status =
        cublasXhpr(reinterpret_cast<cublasHandle_t>(args.cublas_handle), args.layout, convertToCUBLAS(args.triangle),
                   args.n, args.alpha, buffers.x_vec, args.x_offset, args.x_inc, buffers.ap_mat, args.ap_offset);
  ...
  static size_t GetFlops(const Arguments<U>& args) { return 3 * ((args.n * (args.n + 1)) / 2); }
  static size_t GetBytes(const Arguments<U>& args) { return ((args.n * (args.n + 1)) + args.n) * sizeof(T); }
};
