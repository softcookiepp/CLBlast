...
class TestXsbmv {
 public:
  ...
  static std::vector<std::string> GetOptions() {
    return {kArgN,    kArgKL,      kArgLayout,  kArgTriangle, kArgALeadDim, kArgXInc,
            kArgYInc, kArgAOffset, kArgXOffset, kArgYOffset,  kArgAlpha,    kArgBeta};
  }
  ...
  static StatusCode RunRoutine(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto event = cl_event{};
    auto status = Sbmv(args.layout, args.triangle, args.n, args.kl, args.alpha, buffers.a_mat(), args.a_offset,
...
// Describes how to run the clBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference1(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    auto queue_plain = queue();
    auto event = cl_event{};
    auto status = clblasXsbmv(convertToCLBLAS(args.layout), convertToCLBLAS(args.triangle), args.n, args.kl, args.alpha,
...
// Describes how to run the CPU BLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference2(const Arguments<T>& args, BuffersHost<T>& buffers_host, Queue&) {
    cblasXsbmv(convertToCBLAS(args.layout), convertToCBLAS(args.triangle), args.n, args.kl, args.alpha,
               buffers_host.a_mat, args.a_offset, args.a_ld, buffers_host.x_vec, args.x_offset, args.x_inc, args.beta,
...
// Describes how to run the cuBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference3(const Arguments<T>& args, BuffersCUDA<T>& buffers, Queue&) {
    auto status =
        cublasXsbmv(reinterpret_cast<cublasHandle_t>(args.cublas_handle), args.layout, convertToCUBLAS(args.triangle),
                    args.n, args.kl, args.alpha, buffers.a_mat, args.a_offset, args.a_ld, buffers.x_vec, args.x_offset,
