...
class TestXtrsv {
 public:
  ...
  static std::vector<std::string> GetOptions() {
    return {kArgN,        kArgLayout, kArgTriangle, kArgATransp, kArgDiagonal,
            kArgALeadDim, kArgXInc,   kArgAOffset,  kArgXOffset};
  }
  ...
  static void PrepareData(const Arguments<T>& args, Queue&, const int, std::vector<T>& x_source, std::vector<T>&,
                          std::vector<T>& a_source, std::vector<T>&, std::vector<T>&, std::vector<T>&,
                          std::vector<T>&) {
    if (args.a_ld < args.n) {
      return;
    }
    if (args.a_size <= 0 || args.x_size <= 0) {
      return;
    }
    ...
    for (auto i = size_t{0}; i < args.n; ++i) {
      ...
      diagonal = static_cast<T>(AbsoluteValue(diagonal)) + Constant<T>(static_cast<double>(args.n / size_t{4}));
  ...
  static StatusCode RunRoutine(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto status = Trsv<T>(args.layout, args.triangle, args.a_transpose, args.diagonal, args.n, buffers.a_mat(),
                          args.a_offset, args.a_ld, buffers.x_vec(), args.x_offset, args.x_inc, &queue_plain, &event);
...
// Describes how to run the clBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference1(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto event = cl_event{};
    auto status =
        clblasXtrsv<T>(convertToCLBLAS(args.layout), convertToCLBLAS(args.triangle), convertToCLBLAS(args.a_transpose),
                       convertToCLBLAS(args.diagonal), args.n, buffers.a_mat, args.a_offset, args.a_ld, buffers.x_vec,
                       args.x_offset, args.x_inc, 1, &queue_plain, 0, nullptr, &event);
    clWaitForEvents(1, &event);
...
// Describes how to run the CPU BLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference2(const Arguments<T>& args, BuffersHost<T>& buffers_host, Queue&) {
    cblasXtrsv(convertToCBLAS(args.layout), convertToCBLAS(args.triangle), convertToCBLAS(args.a_transpose),
               convertToCBLAS(args.diagonal), args.n, buffers_host.a_mat, args.a_offset, args.a_ld, buffers_host.x_vec,
               args.x_offset, args.x_inc);
...
// Describes how to run the cuBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference3(const Arguments<T>& args, BuffersCUDA<T>& buffers, Queue&) {
    auto status =
        cublasXtrsv(reinterpret_cast<cublasHandle_t>(args.cublas_handle), args.layout, convertToCUBLAS(args.triangle),
                    convertToCUBLAS(args.a_transpose), convertToCUBLAS(args.diagonal), args.n, buffers.a_mat,
                    args.a_offset, args.a_ld, buffers.x_vec, args.x_offset, args.x_inc);
