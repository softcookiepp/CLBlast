...
class TestXherk {
 public:
  ...
  static std::vector<std::string> GetOptions() {
    return {kArgN,        kArgK,       kArgLayout,  kArgTriangle, kArgATransp, kArgALeadDim,
            kArgCLeadDim, kArgAOffset, kArgCOffset, kArgAlpha,    kArgBeta};
  }
  ...
  static size_t GetSizeC(const Arguments<U>& args) { return args.n * args.c_ld + args.c_offset; }
  ...
  static void PrepareData(const Arguments<U>&, Queue&, const int, std::vector<T>&, std::vector<T>&, std::vector<T>&,
  ...
  static StatusCode RunRoutine(const Arguments<U>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto event = cl_event{};
    auto status =
        Herk(args.layout, args.triangle, args.a_transpose, args.n, args.k, args.alpha, buffers.a_mat(), args.a_offset,
             args.a_ld, args.beta, buffers.c_mat(), args.c_offset, args.c_ld, &queue_plain, &event);
    if (status == StatusCode::kSuccess) {
      clWaitForEvents(1, &event);
      clReleaseEvent(event);
    }
#elif CUDA_API
    auto status = Herk(args.layout, args.triangle, args.a_transpose, args.n, args.k, args.alpha, buffers.a_mat(),
                       args.a_offset, args.a_ld, args.beta, buffers.c_mat(), args.c_offset, args.c_ld,
    ...
    auto status =
        Herk(args.layout, args.triangle, args.a_transpose, args.n, args.k, args.alpha, buffers.a_mat(), args.a_offset,
             args.a_ld, args.beta, buffers.c_mat(), args.c_offset, args.c_ld, queue_plain);
    queue_plain->sync();
#endif
    return status;
  }
...
// Describes how to run the clBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference1(const Arguments<U>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto event = cl_event{};
    auto status =
        clblasXherk(convertToCLBLAS(args.layout), convertToCLBLAS(args.triangle), convertToCLBLAS(args.a_transpose),
                    args.n, args.k, args.alpha, buffers.a_mat, args.a_offset, args.a_ld, args.beta, buffers.c_mat,
                    args.c_offset, args.c_ld, 1, &queue_plain, 0, nullptr, &event);
    clWaitForEvents(1, &event);
...
// Describes how to run the CPU BLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference2(const Arguments<U>& args, BuffersHost<T>& buffers_host, Queue&) {
    cblasXherk(convertToCBLAS(args.layout), convertToCBLAS(args.triangle), convertToCBLAS(args.a_transpose), args.n,
               args.k, args.alpha, buffers_host.a_mat, args.a_offset, args.a_ld, args.beta, buffers_host.c_mat,
...
// Describes how to run the cuBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference3(const Arguments<U>& args, BuffersCUDA<T>& buffers, Queue&) {
    auto status =
        cublasXherk(reinterpret_cast<cublasHandle_t>(args.cublas_handle), args.layout, convertToCUBLAS(args.triangle),
                    convertToCUBLAS(args.a_transpose), args.n, args.k, args.alpha, buffers.a_mat, args.a_offset,
                    args.a_ld, args.beta, buffers.c_mat, args.c_offset, args.c_ld);
  ...
  static size_t GetFlops(const Arguments<U>& args) { return args.n * args.n * args.k; }
  static size_t GetBytes(const Arguments<U>& args) { return (args.n * args.k + args.n * args.n) * sizeof(T); }
};
