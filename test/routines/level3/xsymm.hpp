...
class TestXsymm {
 public:
  ...
  static std::vector<std::string> GetOptions() {
    return {kArgM,        kArgN,       kArgLayout,  kArgSide,    kArgTriangle, kArgALeadDim, kArgBLeadDim,
  ...
  static StatusCode RunRoutine(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto event = cl_event{};
    auto status = Symm(args.layout, args.side, args.triangle, args.m, args.n, args.alpha, buffers.a_mat(),
...
// Describes how to run the clBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference1(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto event = cl_event{};
    auto status =
        clblasXsymm(convertToCLBLAS(args.layout), convertToCLBLAS(args.side), convertToCLBLAS(args.triangle), args.m,
                    args.n, args.alpha, buffers.a_mat, args.a_offset, args.a_ld, buffers.b_mat, args.b_offset,
                    args.b_ld, args.beta, buffers.c_mat, args.c_offset, args.c_ld, 1, &queue_plain, 0, nullptr, &event);
    clWaitForEvents(1, &event);
...
// Describes how to run the CPU BLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference2(const Arguments<T>& args, BuffersHost<T>& buffers_host, Queue&) {
    cblasXsymm(convertToCBLAS(args.layout), convertToCBLAS(args.side), convertToCBLAS(args.triangle), args.m, args.n,
               args.alpha, buffers_host.a_mat, args.a_offset, args.a_ld, buffers_host.b_mat, args.b_offset, args.b_ld,
               args.beta, buffers_host.c_mat, args.c_offset, args.c_ld);
...
// Describes how to run the cuBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference3(const Arguments<T>& args, BuffersCUDA<T>& buffers, Queue&) {
    auto status =
        cublasXsymm(reinterpret_cast<cublasHandle_t>(args.cublas_handle), args.layout, convertToCUBLAS(args.side),
                    convertToCUBLAS(args.triangle), args.m, args.n, args.alpha, buffers.a_mat, args.a_offset, args.a_ld,
  ...
  static size_t GetResultIndex(const Arguments<T>& args, const size_t id1, const size_t id2) {
    return (args.layout == Layout::kRowMajor) ? id1 * args.c_ld + id2 + args.c_offset
