...
class TestXconvgemm {
 public:
  ...
  static std::vector<std::string> GetOptions() {
    return {kArgKernelMode, kArgChannels,   kArgHeight,  kArgWidth,   kArgKernelH,   kArgKernelW,
            kArgPadH,       kArgPadW,       kArgStrideH, kArgStrideW, kArgDilationH, kArgDilationW,
            kArgNumKernels, kArgBatchCount, kArgAOffset, kArgBOffset, kArgCOffset};
  }
  ...
  static size_t NumPatches(const Arguments<T>& args) { return OutputHeight(args) * OutputWidth(args) * args.channels; }
  ...
  static StatusCode RunRoutine(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto status = Convgemm<T>(args.kernel_mode, args.channels, args.height, args.width, args.kernel_h, args.kernel_w,
                              ...
                              args.b_offset, buffers.c_mat(), args.c_offset, &queue_plain, &event);
    ...
    for (auto co_id = size_t{0}; co_id < args.num_kernels; ++co_id) {  // output channels == num-kernels
      for (auto ho_id = size_t{0}; ho_id < output_h; ++ho_id) {        // image height
        for (auto wo_id = size_t{0}; wo_id < output_w; ++wo_id) {      // image width
          ...
          for (auto ci_id = size_t{0}; ci_id < args.channels; ++ci_id) {      // input channels
            for (auto kh_id = size_t{0}; kh_id < args.kernel_h; ++kh_id) {    // kernel height
              for (auto kw_id = size_t{0}; kw_id < args.kernel_w; ++kw_id) {  // kernel width
                ...
                if (hi_id >= 0 && hi_id < args.height && wi_id >= 0 && wi_id < args.width) {
                  const auto input_index =
                      wi_id + args.width * (hi_id + args.height * (ci_id + args.channels * (batch_id)));
                  ...
                  const auto kernel_index =
                      (args.kernel_mode == KernelMode::kConvolution)
                          ? (args.kernel_w - kw_id - 1) +
                                args.kernel_w *
                                    ((args.kernel_h - kh_id - 1) + args.kernel_h * (ci_id + args.channels * (co_id)))
                          : kw_id + args.kernel_w * (kh_id + args.kernel_h * (ci_id + args.channels * (co_id)));
          ...
          const auto output_index = wo_id + output_w * (ho_id + output_h * (co_id + args.num_kernels * (batch_id)));
...
StatusCode RunReference<half>(const Arguments<half>& args, BuffersHost<half>& buffers_host) {
  ...
  args2.a_size = args.a_size;
  args2.b_size = args.b_size;
  args2.c_size = args.c_size;
  args2.kernel_mode = args.kernel_mode;
  args2.channels = args.channels;
  args2.height = args.height;
  args2.width = args.width;
  args2.kernel_h = args.kernel_h;
  args2.kernel_w = args.kernel_w;
  args2.pad_h = args.pad_h;
  args2.pad_w = args.pad_w;
  args2.stride_h = args.stride_h;
  args2.stride_w = args.stride_w;
  args2.dilation_h = args.dilation_h;
  args2.dilation_w = args.dilation_w;
  args2.num_kernels = args.num_kernels;
  args2.batch_count = args.batch_count;
  args2.a_offset = args.a_offset;
  args2.b_offset = args.b_offset;
  args2.c_offset = args.c_offset;
