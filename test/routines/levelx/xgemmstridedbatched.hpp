  ...
  static StatusCode RunRoutine(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto status =
        GemmStridedBatched(args.layout, args.a_transpose, args.b_transpose, args.m, args.n, args.k, args.alpha,
                           ...
                           args.c_ld, PerBatchSizeC(args), args.batch_count, &queue_plain, &event);
  ...
  static StatusCode RunReference1(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    for (auto batch = size_t{0}; batch < args.batch_count; ++batch) {
      ...
      auto status = clblasXgemm(convertToCLBLAS(args.layout), convertToCLBLAS(args.a_transpose),
                                convertToCLBLAS(args.b_transpose), args.m, args.n, args.k, args.alpha, buffers.a_mat,
                                a_batch_offset, args.a_ld, buffers.b_mat, b_batch_offset, args.b_ld, args.beta,
                                buffers.c_mat, c_batch_offset, args.c_ld, 1, &queue_plain, 0, nullptr, &event);
      clWaitForEvents(1, &event);
...
// Describes how to run the cuBLAS routine (for correctness/performance comparison)
  ...
  static StatusCode RunReference3(const Arguments<T>& args, BuffersCUDA<T>& buffers, Queue&) {
    for (auto batch = size_t{0}; batch < args.batch_count; ++batch) {
      ...
      auto status = cublasXgemm(reinterpret_cast<cublasHandle_t>(args.cublas_handle), args.layout,
                                ...
                                args.k, args.alpha, buffers.a_mat, a_batch_offset, args.a_ld, buffers.b_mat,
                                b_batch_offset, args.b_ld, args.beta, buffers.c_mat, c_batch_offset, args.c_ld);
  ...
  static size_t GetResultIndex(const Arguments<T>& args, const size_t id1, const size_t id2_3) {
    ...
    return (args.layout == Layout::kRowMajor) ? id1 * args.c_ld + id2 + c_batch_offset
