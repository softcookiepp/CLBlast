...
class TestXim2col {
 public:
  ...
  static std::vector<std::string> GetOptions() {
    return {kArgKernelMode, kArgChannels, kArgHeight,  kArgWidth,     kArgKernelH,   kArgKernelW, kArgPadH,
            kArgPadW,       kArgStrideH,  kArgStrideW, kArgDilationH, kArgDilationW, kArgAOffset, kArgBOffset};
  }
  ...
  static size_t GetSizeA(const Arguments<T>& args) { return args.height * args.width * args.channels + args.a_offset; }
  ...
  static StatusCode RunRoutine(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto status = Im2col<T>(args.kernel_mode, args.channels, args.height, args.width, args.kernel_h, args.kernel_w,
                            ...
                            buffers.a_mat(), args.a_offset, buffers.b_mat(), args.b_offset, &queue_plain, &event);
...
#elif CUDA_API
    auto status =
        Im2col<T>(args.kernel_mode, args.channels, args.height, args.width, args.kernel_h, args.kernel_w, args.pad_h,
  ...
  static size_t GetFlops(const Arguments<T>&) { return 1; }
      ...
      for (auto kw_id = size_t{0}; kw_id < args.kernel_w; ++kw_id) {  // kernel width
        for (auto h_id = size_t{0}; h_id < col_h; ++h_id) {           // image height
          for (auto w_id = size_t{0}; w_id < col_w; ++w_id) {         // image width
            ...
            if (h_index >= 0 && h_index < args.height && w_index >= 0 && w_index < args.width) {
              const auto im_index = w_index + args.width * (h_index + args.height * c_id);
              val = buffers_host.a_mat[im_index + args.a_offset];
            }

            // Sets the output value
            const auto kernel_index = (args.kernel_mode == KernelMode::kConvolution)
            ...
            const auto col_index =
                patch_index + kernel_index * col_w * col_h + c_id * col_w * col_h * args.kernel_h * args.kernel_w;
