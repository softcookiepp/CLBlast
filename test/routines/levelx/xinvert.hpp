...
StatusCode RunReference<half>(const Arguments<half>& args, BuffersHost<half>& buffers_host) {
  ...
  args2.triangle = args.triangle;
  args2.diagonal = args.diagonal;
...
class TestXinvert {
 public:
  ...
  static std::vector<std::string> GetOptions() {
    return {kArgN, kArgM, kArgLayout, kArgTriangle, kArgDiagonal, kArgALeadDim, kArgAOffset};
  }
  ...
  static size_t GetBytes(const Arguments<T>& args) { return (args.a_size * args.b_size) * sizeof(T); }
};
