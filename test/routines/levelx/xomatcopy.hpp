...
StatusCode RunReference(const Arguments<T>& args, BuffersHost<T>& buffers_host) {
  ...
  if ((args.a_ld < args.m && !a_rotated) || (args.a_ld < args.n && a_rotated)) {
    return StatusCode::kInvalidLeadDimA;
  }
  if ((args.b_ld < args.m && !b_rotated) || (args.b_ld < args.n && b_rotated)) {
    return StatusCode::kInvalidLeadDimB;
  }
  if (buffers_host.a_mat.size() * sizeof(T) < (a_base + args.a_offset) * sizeof(T)) {
    return StatusCode::kInsufficientMemoryA;
  }
  if (buffers_host.b_mat.size() * sizeof(T) < (b_base + args.b_offset) * sizeof(T)) {
    return StatusCode::kInsufficientMemoryB;
  }
...
StatusCode RunReference<half>(const Arguments<half>& args, BuffersHost<half>& buffers_host) {
  ...
  args2.a_ld = args.a_ld;
  args2.b_ld = args.b_ld;
  args2.m = args.m;
  args2.n = args.n;
...
class TestXomatcopy {
 public:
  ...
  static std::vector<std::string> GetOptions() {
    return {kArgM, kArgN, kArgLayout, kArgATransp, kArgALeadDim, kArgBLeadDim, kArgAOffset, kArgBOffset, kArgAlpha};
  }
  ...
  static StatusCode RunRoutine(const Arguments<T>& args, Buffers<T>& buffers, Queue& queue) {
    ...
    auto status = Omatcopy<T>(args.layout, args.a_transpose, args.m, args.n, args.alpha, buffers.a_mat(), args.a_offset,
                              args.a_ld, buffers.b_mat(), args.b_offset, args.b_ld, &queue_plain, &event);
  ...
  static size_t GetFlops(const Arguments<T>& args) { return args.m * args.n; }
  static size_t GetBytes(const Arguments<T>& args) { return (2 * args.m * args.n) * sizeof(T); }
};
