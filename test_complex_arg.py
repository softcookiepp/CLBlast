...
with open("test_complex_arg.comp", "r") as f:
	src = f.read()

module = dev.compile_glsl(src)
pipeline = dev.create_pipeline(module, "main")

a = np.random.randn(256).astype(np.complex64)
a_buf = dev.allocate_buffer(a.nbytes)
a_buf.copy_in(a)

push = np.zeros(5).astype(np.int32)
push[0] = 256
push.view(np.float32)[1] = 2.0
push.view(np.float32)[2] = 5.0
push[3] = 0
push[4] = 1

sequence = dev.create_sequence()
sequence.record_pipeline(pipeline, [4, 1, 1], [a_buf], push)
